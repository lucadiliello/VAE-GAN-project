...
class Encoder(nn.Module):
    ...
    def __init__(self, ngf=32, z_dim=512):
        ...
        self.fc_mu = nn.Conv2d(self.f_dim * 16, self.z_dim, kernel_size=4, stride=1, padding=0)    
        self.fc_logvar = nn.Conv2d(self.f_dim * 16, self.z_dim, kernel_size=4, stride=1, padding=0)
    

    def forward(self, img):
        # img has shape [bs, channels, height, width]
        e0 = self.conv0(img)
        e1 = self.conv1(e0)
        e2 = self.conv2(e1)
        e3 = self.conv3(e2)
